...
//  $sql = "select count(*) as cnt from post order by post_id desc";
  ...
  if ($allpost == 0) {
    $emptyData = '<tr><td class = "textCenter" colspan = "10">글이 존재하지 않습니다.</td></tr>';
  } else {
            ...
            while($row = $result -> fetch(PDO::FETCH_ASSOC)) {  //출력을 받습니다.
              ...
              $title = $row["title"]." [$comment_count]";
					  ...
					  <tr>
						...
						<td class ="name"><a href ="./view.php?title=<?= $row["title"];?>&post_id=<?= $post_id ?>&dept_name=<?= $dept_name ?>"><?= $title ?></a></td> <!-- post id도 보내야함-->
