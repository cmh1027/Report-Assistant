...
<?php
 ...
 <div class="container-fluid">
     <div class="row content">
       <div class="col-sm-1 sidenav"  id="view_side">
   ...
   <?php
      try{
       ...
       ?>
   <div id="view_main" class ="col-sm-11">
	   <h4 id="view_title"><p>제목: <text> <?= $var["title"]?> </text> </p></h4>
	<h4 id="view_writer"><small>작성자: <text> <?= $var["nickname"]?> </small></text></h4>
	<div class="post_content"><?= $var["content"]?> </div><br/>
<?php if($nick == $var["nickname"] ) { ?>
     <form action="../writing.php" method="POST">
	   ...
	   <input type="hidden" name="post_id" value="<?= $post_id?>"></input>
		 <input type="hidden" name="modify" value="true"></input>
	   <input class="btn" type= "submit" value="수정하기"/> <!-- 글작성자의 소유인경우만 보여줄것-->
	 </form>
<?php } ?><br/>
<?php if($nick == $var["nickname"] ) { ?>
     <a href = "./delete.php?title=<?= $title?>&post_id=<?= $post_id?>&dept_name=<?= $dept_name ?>"><button class="btn">삭제하기</button></a><!-- 글작성자의 소유인경우만 보여줄것-->
<?php } ?><br/><br/>
     <a href = "./mainboard.php"><button class="btn">목록</button></a><br/><br/>
		  ...
		  <table>
			<tr>
			 <form action = "comment_update.php" method= "post">
				...
				<textarea id="view_txt" cols ="100" rows ="3" name ="comment"></textarea>
			  <div class = "sub_but">
				<input type = "submit" class="btn" name ="submit" value = "등록">
			  </div>
			 </form>
			</tr>
		</table>
				...
				<form action = "comment_delete.php" method = "post" >
				  ...
				  <div class ="sub_but">
					<?php if($nick == $var_co["comment_nickname"] ) { ?>
					  <input type = "submit" class="btn" name ="submit" value ="삭제" >
					 <?php } ?>
				  </div>
				</form>
			  </div>
